...
class BehaviorCNN(nn.Module):
    def __init__(self, input_size=8, num_classes=4, window_length=10):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)  # (batch, input_size, window_length)
        x = self.conv1(x)
        x = self.relu1(x)
        x = self.conv2(x)
        x = self.relu2(x)
...
def plot_and_save_training_curves(history, save_path='training_curves_cnn.png'):
...
def run_cnn_experiment(X_raw, y_raw, window_length, batch_size, epochs, device, num_classes, include_base=True):
    ...
    # Model
    model = BehaviorCNN(input_size=num_features, num_classes=num_classes, window_length=window_length).to(device)
    ...
    plot_and_save_training_curves(history, save_path=f'training_curves_cnn_{"4class" if include_base else "3class"}.png')
...
def main():
    ...
    # 4-class experiment
    run_cnn_experiment(X_raw, y_raw, WINDOW_LENGTH, batch_size=BATCH_SIZE, epochs=EPOCHS, device=DEVICE, num_classes=4, include_base=True)

    # 3-class experiment
    run_cnn_experiment(X_raw, y_raw, WINDOW_LENGTH, batch_size=BATCH_SIZE, epochs=EPOCHS, device=DEVICE, num_classes=3, include_base=False)
